...
class Stdout_progressbar:
    ...
    def update(self, now_num): # num从1开始
        if now_num == 1:
            ...
            if elapse < 1:
                print(f"{1/elapse:.2f}step(frame) per second, eta {elapse*(self.max_num - 1):.2f}s")
            else:
                print(f"{elapse:.2f}second per step(frame), eta {elapse*(self.max_num - 1):.2f}s")
...
if __name__ == '__main__':
    stdoutpb = Stdout_progressbar(10)
    stdoutpb.reset()
    for i in range(10):
        time.sleep(0.5)
        stdoutpb.update(i+1)
