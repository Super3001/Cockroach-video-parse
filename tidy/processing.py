...
def rect_cover1(frame, thres_value=0, edge=0):
    """在指定的图像中用一个矩形覆盖所有值大于thres_value的点
    """
...
def main_color(cap,kind,root,OutWindow,progressBar,pm=1,skip_n=1):  # 颜色提取
    ...
    while success:
        ...
        rtn, center = color_deal(frame[domain[0]:domain[1]+1,domain[2]:domain[3]+1],midval, 15, pre_state, thresh_dist, OutWindow)
        if rtn==0:
            pre_state = -1
